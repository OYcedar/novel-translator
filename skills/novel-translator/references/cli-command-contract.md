...
| `verify-delivery --manifest <目录>/delivery-manifest.json --json` | 校验交付包 manifest 中记录的相对路径、文件大小和 SHA256 | `status` 为 `ok` 且 `summary.errors` 为 0 | error 表示交付文件缺失或被改动，必须重新生成或更正交付包；交付目录移动后仍应可校验 |
