...
3. 最后处理源文残留。确认是人名、拟声词、品牌名等应保留片段时，在术语表或配置中说明；不要全局关闭检查来掩盖漏翻。
...
当前版本还没有独立的 `export-quality-fix`、`import-manual-translations`、`reset-translations` 和 `verify-feedback-text` 命令。需要人工修复时，应优先通过后续工具命令实现；在这些命令未实现前，不要直接编辑缓存伪造状态，除非用户明确要求进行工具开发或数据抢救。
